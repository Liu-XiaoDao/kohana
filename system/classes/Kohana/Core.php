...
class Kohana_Core {
	 ...
	 */
	public static $log;//日志对象

	/**
	 * @var  Config  config object
	 */
	public static $config;//配置对象

	/**
	 * @var  boolean  Has [Kohana::init] been called?
	 */
	protected static $_init = FALSE;//[Kohana::init]是否被调用

	/**
	 * @var  array   Currently active modules
	 */
	protected static $_modules = array();//当前可用模块

	/**
	 * @var  array   Include paths that are used to find files
	 */
	protected static $_paths = array(APPPATH, SYSPATH);//用于查找文件的路径
	 ...
	 */
	protected static $_files_changed = FALSE;//文件缓存路径被更改了吗
	...
	public static function init(array $settings = NULL)
	{
		...
		Kohana::$_init = TRUE;//走到这一步_init原本就应该是true

		if (isset($settings['profile']))
		{
			// Enable profiling
			Kohana::$profiling = (bool) $settings['profile'];//这不现情况应该不走
		}

		// Start an output buffer
		ob_start();//开启一个输出缓冲区，不知道问什么能用

		if (isset($settings['errors']))//不走
		{
		...
		if (Kohana::$errors === TRUE)
		{
			...
			set_exception_handler(array('Kohana_Exception', 'handler'));//自定义异常处理

			// Enable Kohana error handling, converts all PHP errors to exceptions.
			set_error_handler(array('Kohana', 'error_handler'));//自定义错误处理
		}
		...
		{
		    ini_set('xdebug.collect_params', 3);//设置调试为3
		}

		// Enable the Kohana shutdown handler, which catches E_FATAL errors.
		register_shutdown_function(array('Kohana', 'shutdown_handler'));//程序崩溃调用程序？？

		if (ini_get('register_globals'))//如果设置本地化，调用本类的本地化函数
		{
		...
		Kohana::$is_windows = (DIRECTORY_SEPARATOR === '\\');//在window使用反斜线

		// Determine if we are running in safe mode
		Kohana::$safe_mode = (bool) ini_get('safe_mode');//设置安全模式

		if (isset($settings['cache_dir']))//关于缓存
		{
			...
			Kohana::$cache_dir = realpath($settings['cache_dir']);//拿到缓存绝对路径
		}
		else
		{
			// Use the default cache directory
			Kohana::$cache_dir = APPPATH.'cache';//未设置用默认
		}

		if ( ! is_writable(Kohana::$cache_dir))//如果不能写报错
		{
			throw new Kohana_Exception('Directory :dir must be writable',//自定义异常
				array(':dir' => Debug::path(Kohana::$cache_dir)));
		}

		if (isset($settings['cache_life']))//缓存时间
		{
			// Set the default cache lifetime
			Kohana::$cache_life = (int) $settings['cache_life'];
		}

		if (isset($settings['caching']))//是否缓存
		{
		...
		if (Kohana::$caching === TRUE)
		{
			...
			Kohana::$_files = Kohana::cache('Kohana::find_file()');//如果缓存拿到缓存文件
		}

		if (isset($settings['charset']))
		{
			// Set the system character set
			Kohana::$charset = strtolower($settings['charset']);//设置字符集
		}

		if (function_exists('mb_internal_encoding'))//如果存在这个方法设置什么（字符）
		{
			// Set the MB extension encoding to the same character set
			mb_internal_encoding(Kohana::$charset);
		}

		if (isset($settings['base_url']))//设置根目录
		{
			// Set the base URL
			Kohana::$base_url = rtrim($settings['base_url'], '/').'/';
		}

		if (isset($settings['index_file']))//设置index
		{
			// Set the index file
		...
		Kohana::$magic_quotes = (bool) get_magic_quotes_gpc();//？？魔法引号

		// Sanitize all request variables
		$_GET    = Kohana::sanitize($_GET);//整理请求数组
		$_POST   = Kohana::sanitize($_POST);
		$_COOKIE = Kohana::sanitize($_COOKIE);

		// Load the logger if one doesn't already exist
		if ( ! Kohana::$log instanceof Log)//使用正确LOG类
		{
	 ...
	 * @return  void
	 */
	public static function deinit()//应该是取消自己的初始化
	{
		if (Kohana::$_init)
		{
	 ...
	 * @return  void
	 */
	public static function globals()//全局
	{
	 ...
	 */
	public static function sanitize($value)//净化数组
	{
	 ...
	 * @return  boolean
	 */
	public static function auto_load($class, $directory = 'classes')//自动加载
	{
	 ...
	 * @return  array
	 */
	public static function include_paths()//返回所有路径
	{
	 ...
	 */
	public static function find_file($dir, $file, $ext = NULL, $array = FALSE)//找到并返回一个文件
	{
		if ($ext === NULL)
		{
	 ...
	 * @return  array
	 */
	public static function list_files($directory = NULL, array $paths = NULL)//拿到一个目录下所有文件
	{
		if ($directory !== NULL)
		{
	 ...
	 * @return  mixed
	 */
	public static function load($file)//include一个文件
	{
		return include $file;
	}
	 ...
	 */
	public static function cache($name, $data = NULL, $lifetime = NULL)//缓存
	{
